...
- $\exists \; i \; s.t. \; |\lambda_i|=1 \; \wedge \;|\lambda_i|\leq 1 \ \ \ \forall i$, the fixed point is **marginal**, linear analysis is inconclusive and higher order terms $\mathcal{O}\left(\|\hat{\phi}_t \|^2\right)$ need to be considered.
...
### Complex roots
Suppose $a^2-4b< 0$, then the roots $\lambda_{1,2}$ are complex conjugates. Since $b$ is equal to the product of the roots (and they have the magnitude), we find that 
$$
...
Note that the above inequality also gives us,
$$
1+b > |a| \geq 0 \quad \Rightarrow \quad b > -1,
$$
Hence, rather than stating $|b| < 1$ from the start of the proof it is sufficient to use the simpler $b < 1$, as the last two Jury conditions imply $b>1$ regardless.

###  Real roots 
Suppose $a^2-4b\geq 0$ such that the roots $\lambda_{1,2} \in \mathbb{R}$ and take $R$ to be the largest of their absolute values,
$$
R=\max\{|\lambda_1|,|\lambda_2|\} =  \frac{|a| + \sqrt{a^2-4b}}{2}.
$$ {#eq-noLabel}
This is an increasing function of $|a|$. Thus we can bound $a$ by considering $R=1$,
$$
\begin{aligned}
R=1 \; & \Rightarrow \; \frac{|a| + \sqrt{a^2-4b}}{2}=1,\\
& \Rightarrow \; |a|-2=-\sqrt{|a|^2-4b},\\
& \Rightarrow \; |a|^2-4|a|+4=|a|^2-4b,\\
& \Rightarrow \; |a|=b+1.
\end{aligned}
$$ {#eq-noLabel}
Hence $0\leq R < 1$ if and only if $0\leq |a|<1+b$. Also, since $|\lambda_i|<1 \forall i$ implies that $|\lambda_1\lambda_2|<1$, it follows that $|b|<1$ must hold. Similar to the argument for complex roots $b>-1$ is already implied by the last two Jury conditions thus it is sufficent to take $b<1$.
