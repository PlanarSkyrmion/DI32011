...
Let $N_t$ be a dependent variable, representing the population size of a single species, at the discrete time $t\in\mathbb{N}$. We write the change in population as the first order difference equation,
$$
N_{t+1} = H(N_t) = N_tf(N_t),
$$ {#eq-GeneralPopulationModel} 
where $H(N_t)$ describes the total net growth rate and $f(N_t)$ defines the *net per capita* growth rate.
...
Finally, note that many of you were introduced to difference equations in the module MA21003 Discrete Mathematics. It is recommended that you revisit the section on Discrete Difference Equations in your notes.

## The Malthusian model

The simplest form for @eq-GeneralPopulationModel is a *constant net per capita growth rate*. Let $b$ be the per capita growth rate and $d$ the per capita death rate where $b,d \in \mathbb{R}_{\geq 0}$ ($b$ and $d$ are both positive real constants) and $d \leq 1$ (it's not possible for more than 100 \% of the population to die). This gives the difference equations as,
$$
...
**Exercise:** Describe how the behaviour of the Malthusian model changes with $r$. What are the critical values of $r$ that split the Malthusian model into qualitatively different behaviour?
...
$$ 
where $r, K, n \in \mathbb{R}_{> 0} are positive parameters. Below we consider the Ricker model which was initially used to study Canadian sockeye salmon population dynamics.
