...
H'(0)= \gamma.
$$ {#eq-noLabel}
Hence if $\gamma < 1$ the first fixed point is monotonically stable as $0< H'(0) < 1$. When $\gamma > 1$ the first fixed point becomes montonically unstable, also the second fixed point becomes biologically relevant.
...
gamma_greater1 = np.linspace(1, 2, 200) # N^*=0 unstable, non-zero N^* montonically stable
...
3. $\boldsymbol{h < \gamma < h+1} \; \Rightarrow \; 0 < H'(0) < 1$, $\, N^*_1$ is montonically stable  $\rightarrow$ population collapse, 
4. $\boldsymbol{h+1 < \gamma < \frac{2(h+1)^2}{2h+1}} \; \Rightarrow \; 0 < H'(N^*_2) < 1$, $\, $N^*_2$ is montonically stable $\rightarrow$ population stable,
...
### Return to the harvesting model example
