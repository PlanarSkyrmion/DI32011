...
where the function $H(N_t)$ describes the total net growth rate and $f(N_t)$ defines the *net per capita* growth rate.
...
The simplest form for @eq-GeneralPopulationModel is a *constant net per capita growth rate*. Let $b$ be the per capita growth rate and $d$ the per capita death rate where $b,d \in \mathbb{R}^+$ ($b$ and $d$ are both positive real constants) and $d \leq 1$ (it's not possible for more than 100 \% of the population to die). This gives the difference equations as,
$$
N_{t+1}=N_t + bN_t - dN_t=rN_t,
$$ {#eq-MalthusianEqn} 
where the parameter $r=1+b-d$ is the *net per capita growth rate*. Note that $d \leq 1$ and $b \geq 0$ implies that $r \geq 0$.
...
# Stable (r < 0)
plt.plot(r[r < 0], N_star[r < 0], 'b', linewidth=3, label='Stable')

# Unstable (r > 0)
plt.plot(r[r > 0], N_star[r > 0], 'r', linewidth=3, label='Unstable')
...
ax.plot(np.concatenate((gamma_greater1
,gamma_greater2)), N_star_unstable0, 'r')
...
stable_osc_handle = Line2D([0], [0], color='blue', lw=2, label='Oscillatory unstable')
...
6. $\boldsymbol{\gamma >\frac{(1+h)^2}{h}}\; \Rightarrow \; H'(N^*_2) < -1$, $N^*_2$ is oscillatory unstable $\rightarrow$ population won't collapse but isn't stable.
...
#| fig-cap: "Bifurcation diagram showing how possible long time populations $N_\\infty$ change for $t > 300$ with $h$ for a fixed $\\gamma = 6$."
import numpy as np
import matplotlib.pyplot as plt
