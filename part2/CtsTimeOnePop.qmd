...
The solution can be found using seperation of variables,
$$
N(t)=N_0\exp(rt),
$$ {#eq-noLabel}
where $N_0$ is the initial population at time $t=0$. We can describe qualitatively different solutions behaviours,
...
We denote **steady-state solutions** or **equilibria** of @eq-gensinglespecodeagain using $N=N^*$. At a steady-state,
$$
\frac{dN}{dt}=0,
$$ {#eq-noLabel}
...
H(N^*)=0.
$$ {#eq-noLabel}
Note, some texts may call these fixed points or critical points. The first is often used in dynamical systems, while the second is older and rarely used as it can be ambiguous.

### Linear stability analysis
To perform a linear stability analysis near an equillibrium we make the change of variables,
$$
N(t)=N^*+\hat{N}(t),
$$ {#eq-noLabel}
where the new dependent variable, $\hat{N}(t)$, is a perturbation about the steady state $N^*$. The time derivative on the left-hand side of @eq-gensinglespecodeagain transforms to,
$$
...
Employing a Taylor expansion on the right-hand side of @eq-gensinglespecodeagain we get,
$$
\frac{d\hat{N}(t) }{dt} = H(N^*)+  H'(N^*)\hat{N}(t) + \frac{1}{2}H''(N^*)\hat{N}^2(t) + \mathcal{O}(\hat{N}^3),
$$ {#eq-noLabel}
where we use $H(N^*)=0$ and assume the perturbation $\hat{N}$ is small to neglect all terms quadratic and higher, which yields the linear ODE,
$$
\frac{d\hat{N}(t) }{dt} =  H'(N^*)\hat{N}(t)
$$ {#eq-noLabel}
with solution (by separation of variables),
$$
\hat{N}(t)=  \eta e^{H'(N^*) t}
$$ {#eq-noLabel}
where $\eta$ is some initial perturbation about the steady-state. We now have 3 phases in this linear regime,
...
#| fig-cap: "Animation demonstrating hysteresis as $r$ increases (picks left stable critical point) and decreases (picks right stable critical point)."
#| fig-show: false
...
\lambda=-e^{-\lambda T}.
$$ {#eq-noLabel}
This equation has no  solutions for $\lambda \in \Re^+$. Hence any steady-states with real eigenvalues are linearly stable.

Now consider solutions $\lambda \in \mathbb{C}$.
...
If $\omega$ is a solution of @eq-mu and @eq-om then so is $-\omega$. Hence w.l.o.g we consider the case $\omega>0$.

Consider $\mu=0$ in @eq-mu. This implies
$$
\omega T=\frac{\pi}{2} + 2n\pi, \quad n \in Z.
$$ {#eq-noLabel}
We consider the smallest positive root, i.e. $n=0$, as we want to consider the smallest value of delay time, $T$, at which instability arises.
...
On the stablity boundary, $\mu=0$. Upon substitution in @eq-om
$$
