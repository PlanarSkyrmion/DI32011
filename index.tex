...
\textbf{Office Hour:} Wednesday 17:00 -- 18:00, {[}Teams Link{]}
...
available
\href{https://www.twinyard.co.uk/DI32011/assets/pythonIntro.pdf}{here}.
...
that changes occur infinitesimally, or discrete, \(t \in \mathbb{Z}\),
...
though there may be other variables involved or multiple competing
populations.
...
\frac{dN}{dt} = (b-d)N = rN.
}\end{equation} For convenience we will define the constant \(r = b-d\).
The solution follows from previous courses on ODEs,
...
of \(r\)) indefinitely. Unsurprisingly such a simplistic model is rarely
accurate.
...
\(r\) increases with time) and since then \(r\) has started to decrease.
It is even predicted that the exponent may become negative in the future
...
variables (both dependent and independent) dimensionless. In general
this will be the population N

In general this will involve rewriting the population and time as the
dimensionless variables,
\begin{equation}\protect\phantomsection\label{eq-generalNonDim}{
n = \frac{N}{\tilde{N}}, \qquad \tau = \frac{t}{\tilde{T}}.
}\end{equation} such that a general ODE transforms as,
\begin{equation}\protect\phantomsection\label{eq-nonDimTransform}{
\frac{dN}{dt} = H(N(t)) \qquad \longrightarrow \qquad \frac{dn}{d\tau} = \frac{\tilde{T}}{\tilde{N}}\, H\left(  \right).
}\end{equation}

A good example is the logistic equation in
Equation~\ref{eq-logisticRedef}, where we absorb the constants by
choosing \(\tilde{N} = K\) and \(\tilde{T} = 1/r\) such that we define,
\begin{equation}\protect\phantomsection\label{eq-nonDim}{
n = \frac{N}{K}, \qquad \tau = rt,
}\end{equation} which we call the \textbf{dimensionless variables}. The
equation then becomes significantly simpler,
\begin{equation}\protect\phantomsection\label{eq-nonDim}{
\frac{dn}{d\tau} = n(1-n).
}\end{equation}
...
Note, be careful with absorbing signs e.g.~for
Equation~\ref{eq-continousExp}, the exponential growth model, if we
defined \(\tau = r N\) we get the dimensionless solution,
\begin{equation}\protect\phantomsection\label{eq-nonDimExp}{
N(\tau) = N_0 \, e^{\tau}.
}\end{equation} This may be suggestive that population always grows, but
if \(d > b \Leftrightarrow r < 0\) then the rescaling has reversed time.

\subsection{Dimensional analysis}\label{dimensional-analysis}
...
For example, we can deduce that the units of the parameter \(r\) in
Equation~\ref{eq-expSol}
\begin{equation}\protect\phantomsection\label{eq-dim1}{
\frac{dN}{dt}=rN.
}\end{equation} The units of the left-hand term are
\begin{equation}\protect\phantomsection\label{eq-dim2}{
\frac{\# pop density}{\# time},
}\end{equation} thus the units of the right-hand side must match. This
means that \(r\) must have the units,
\begin{equation}\protect\phantomsection\label{eq-dim3}{
\#r = \frac{1}{\#time},
}\end{equation} for dimensional consistency. This is a useful technique
for making sure equations are correct and also gives some physical
intuition as to what different parameters represent. In the above
example \(r\) is inverse in time, so it is describing a rate of change,
which matches our intuiton that it is the difference in birth and death
rate.
...
population size (e.g.~a mutation that evolves from one generation to the
next).
...
a single species, at the discrete time \(t\in\mathbb{Z}\). We write the
...
}\end{equation} where the function \(H(N_t)\) describes the total net
growth rate and \(f(N_t)\) defines the \emph{net per capita} growth
rate.
...
in the module Discrete Mathematics. It is recommended that you revisit
the section on Discrete Difference Equations in your notes.
...
\(b,d \in \mathbb{R}^+\) (\(b\) and \(d\) are both positive real
...
be explicitly calculated for all \(t\). In
Figure~\ref{fig-plotmalthusian} we present a numerical solution of the
model where we have simply iterated through solutions for different
...
\protect\phantomsection\label{exercise-Malthusian}
Describe how the behaviour of the Malthusian model changes with \(r\).
What are the critical values of \(r\) that split the Malthusian model
into qualitatively different behaviour? How does this compare with the
continous exponential ODE in Equation~\ref{eq-continousExp}? Are the
parameters \(r\) in the two equations describing the same thing, how do
they differ?
...
Malthusian model into qualitatively different behaviour? How does this
compare with the continous exponential ODE in
Equation~\ref{eq-continousExp}? Are the parameters \(r\) in the two
equations describing the same thing, how do they differ?
...
\] where \$r, K, n \in \mathbb{R}\^{}+ are positive parameters. Below we
...
\subsubsection{Dimensional analysis}\label{dimensional-analysis-1}
